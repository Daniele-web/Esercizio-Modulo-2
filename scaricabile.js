...
function cercaLavori(queryLocation, queryTitolo) {
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    if (locationLavoroCorrente.includes(queryLocationMin) && titleLavoroCorrente.includes(queryTitoloMin)) {
      ...
      risultati.result.push(risultatoPulito);
      risultati.count++;
    }
  }
  return risultati;
...
function cerca() {
  const queryLocation = document.getElementById("location").value;
  const queryTitolo = document.getElementById("title").value;
  const risultati = cercaLavori(queryLocation, queryTitolo);

  const resultDiv = document.getElementById("result");
  resultDiv.innerHTML = ''; // Pulisci i risultati precedenti

  if (risultati.count > 0) {
      const ul = document.createElement("ul");
      risultati.result.forEach(lavoro => {
